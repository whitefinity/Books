...
<img id="ba" src="./includes/img/bc.png">
<script>$('<p/>',{
...
exit;
	...
	$sql="SELECT users.username,DATE_FORMAT(comments.date,'%D %M %Y %k:%i') as date,comments.comments,comments.parent_id,
	...
	$get_com=array();
	echo "<table class=\"comt2\"><tr class=\"title\"><td class=\"titletd\">
	 Page: ".$page."</td></tr>";
	while($row=mysqli_fetch_assoc($user_com)){
		echo "<tr><td class=\"name\"><a href=\"user.php?user={$row['user_id']}\">"
		.$row['username']."</a> | ".$row['date']."</td></tr>";
		echo "<tr class=\"trcolor\"><td class=\"comfont\">".wordwrap(nl2br($row['comments']),180,"<br>",TRUE)."</td></tr>";	
		}
