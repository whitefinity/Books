...
include("includes/header.php");
	if(isset($_GET['filter'])&&($_GET['filter']=="desc")){
		$filter="ASC";
		 echo "<div class=\"filter\"><a  href=\"index.php?filter=asc\">Asc</a></div>"; 
		}elseif(isset($_GET['filter'])&&($_GET['filter']=="asc")){
		 	echo "<div class=\"filter\"><a href=\"index.php?filter=desc\">Desc</a></div>";
			$filter="DESC";
		}else{
		echo "<div class=\"filter\"><a  href=\"index.php?filter=asc\">Asc</a></div>"; 
		$filter="ASC";
...
<img id="ba" src="./includes/img/ba1.png">
<p class="success" ><?php
...
$results[$row['book_id']]['authors'][$row['author_id']]=$row['author_name'];
	...
	echo "<table>";?>
		<tr class="title"><td class="titletd" colspan="2">Page: <?=$page?></td></tr>
	<?php $count=0;
	foreach($results as $key_id=>$v){
		...
		$count++;
		echo "<td class=\"title\"><a href=\"books.php?book={$key_id}\">".$v['book_title']."</a>";		
